      ...
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="bg-gradient-to-r from-[#006400] to-[#008000] p-6 text-center sticky top-0 z-10">
          <div className="bg-[#ffcc00] w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3">
            {mode === 'login' ? (
              <FaSignInAlt className="text-[#006400] text-2xl" />
            ) : (
              <FaUserPlus className="text-[#006400] text-2xl" />
            )}
          </div>
          ...
          <p className="text-[#ffcc00]/80 text-sm">
            {mode === 'login' 
              ? 'Login to join the diaspora forum' 
          ...
          {mode === 'login' ? (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#006400] hover:bg-[#005a00] text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 hover:shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              <p className="text-center text-sm text-gray-500">
                ...
                <button
                  type="button"
                  onClick={() => { setMode('register'); setLoginForm({ username: '', password: '' }); }}
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-3">
              ...
              <p className="text-center text-sm text-gray-500">
                ...
                <button
                  type="button"
                  onClick={() => { setMode('login'); setRegisterForm({ username: '', email: '', password: '', confirmPassword: '', fullName: '' }); }}
                  className="text-[#006400] hover:underline font-semibold"
                >
                  Login here
                </button>
...
const ForumPage = () => {
  ...
  useEffect(() => {
    if (topics.length > 0) {
      const totalReplies = topics.reduce((sum, topic) => sum + (topic.replyCount || 0), 0);
      setStats({
  ...
  const handleRegister = async (registerData) => {
    ...
    try {
      ...
      if (result.success) {
        setShowAuthModal(false);
        await loadTopics();
        toast.success('Registration successful! Welcome to the forum.');
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
          <div className="bg-gradient-to-r from-[#006400] to-[#008000] p-8 text-center">
            ...
            <p className="text-[#ffcc00]/80 text-sm mt-2">
              Join the community of Ugwunagbo people home and abroad
            </p>
          </div>
          <div className="p-8 text-center">
            <p className="text-gray-600 mb-6">
              Please login or register to participate in the forum discussions.
            </p>
            <div className="space-y-3">
              <button
                onClick={() => { setAuthMode('login'); setShowAuthModal(true); }}
                className="w-full bg-[#006400] hover:bg-[#005a00] text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 hover:shadow-lg flex items-center justify-center gap-2"
              >
                <FaSignInAlt /> Login
              </button>
              <button
                onClick={() => { setAuthMode('register'); setShowAuthModal(true); }}
                className="w-full bg-[#ffcc00] text-[#006400] hover:bg-[#e6b800] font-semibold py-3 px-6 rounded-xl transition-all duration-300 flex items-center justify-center gap-2"
              >
            ...
            <button
              onClick={() => navigate('/')}
              className="mt-6 text-gray-500 hover:text-gray-700 text-sm transition-colors flex items-center justify-center gap-2"
            >
              <FaHome /> Back to Home
            </button>
          </div>
